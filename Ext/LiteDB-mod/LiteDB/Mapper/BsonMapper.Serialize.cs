...
namespace LiteDB
{
    public partial class BsonMapper
    {
        ...
        private BsonDocument SerializeDictionary(Type keyType, Type valueType, IDictionary dict, int depth)
        {
            ...
            _customSerializer.TryGetValue(keyType, out var keyCustom);

            foreach (var key in dict.Keys)
            {
                var value = dict[key];
                o.RawValue[keyCustom != null ? keyCustom(key).ToString() : key.ToString()] = this.Serialize(valueType, value, depth);
            }
