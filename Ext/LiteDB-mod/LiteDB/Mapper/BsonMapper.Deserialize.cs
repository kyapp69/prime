...
namespace LiteDB
{
    public partial class BsonMapper
    {
        ...
        private static Type obidt = typeof(ObjectId);

        private void DeserializeDictionary(Type K, Type T, IDictionary dict, BsonDocument value)
        {
            Func<BsonValue, object> custom;
            _customDeserializer.TryGetValue(K, out custom);

            foreach (var key in value.Keys)
            {
                object Conv() //hh: total hack, no time to consider.
                {
                    return K == obidt ? new ObjectId(key) : Convert.ChangeType(key, K);
                }

                var k = custom!=null ? custom(key) : K.GetTypeInfo().IsEnum ? Enum.Parse(K, key) : Conv();
