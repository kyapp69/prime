...
namespace LiteDB
{
    ...
    public class FileDiskService : IDiskService
    {
        ...
        private LoggerWrap _log; // will be initialize in "Initialize()"
        ...
        public virtual void WritePage(uint pageID, byte[] buffer)
        {
            ...
            _log.Write(LoggerWrap.DISK, "write page #{0:0000} :: {1}", pageID, (PageType)buffer[PAGE_TYPE_POSITION]);
        ...
        public void WriteJournal(ICollection<byte[]> pages)
        {
            ...
            foreach(var buffer in pages)
            {
                ...
                _log.Write(LoggerWrap.JOURNAL, "write page #{0:0000} :: {1}", pageID, pageType);
        ...
        public void ClearJournal()
        {
            if (_journal != null)
            {
                _log.Write(LoggerWrap.JOURNAL, "cleaning journal file");
                _journal.TryUnlock(0, 1);
                _journal.Seek(0, SeekOrigin.Begin);
